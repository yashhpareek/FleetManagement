...
const BookingForm = () => {
    ...
    useEffect(() => {
        const fetchAirportCodes = async () => {
            try {
                const response = await axios.get('https://localhost:7185/api/Airport/airport');
    ...
    const handleChangeAirport = async (e, locationType) => {
        ...
        if (selectedAirport) {
            const { city, state } = selectedAirport;
            ...
            const updatedFormData = {
                ...formData,
                [`${locationKeyPrefix}Location`]: value,
                [`${locationKeyPrefix}City`]: city.cityName,
                [`${locationKeyPrefix}CityId`]: city.cityId,
                [`${locationKeyPrefix}State`]: state.stateName,
                [`${locationKeyPrefix}StateId`]: state.stateId
            };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('https://localhost:7185/api/booking', formData, {
                headers: {
                        ...
                        <div className="card-body">
                            ...
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Form.Group controlId="pickupState" >
                                    ...
                                    <Form.Control type="text" name="pickupState" value={formData.pickupState} style={{backgroundColor:'bisque'}} readOnly />
                                </Form.Group>
                                <Form.Group controlId="pickupCity">
                                    <Form.Label>Pickup City:</Form.Label>
                                    <Form.Control type="text" name="pickupCity" value={formData.pickupCity} style={{backgroundColor:'bisque'}} readOnly />
                                </Form.Group>
                                ...
                                <Form.Group controlId="returnState">
                                    ...
                                    <Form.Control type="text" name="returnState" value={formData.returnState} style={{backgroundColor:'bisque'}} readOnly />
                                </Form.Group>
                                <Form.Group controlId="returnCity">
                                    <Form.Label>Return City:</Form.Label>
                                    <Form.Control type="text" name="returnCity" value={formData.returnCity} style={{backgroundColor:'bisque'}} readOnly />
                                </Form.Group>
