...
const PDFGenerator = () => {
  ...
  const generatePDF = () => {
    ...
    addRow(['Car Name:', carData[0].carName]);
    addRow(['Car Type:', carData[0].carType.carTypeName]);
    addRow(['Number Plate:', carData[0].numberPlate]);
    ...
    addRow(['Booking ID:', selectedInvoice.bookingId]);
    addRow(['Rental Amount:', rentalAmount]);
    addRow(['Total Addon Amount:', totalAddonAmount]);
  ...
  const emailMsg = `
Subject: Car Return Confirmation - ${selectedInvoice.bookingId}
